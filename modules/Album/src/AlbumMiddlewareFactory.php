...
/**
 * AlbumMiddlewareFactory based on Expressive/AppFactory.
 *
 * Removed deprecated parts, changed signature of `injectRoutesAndPipeline` from ContainerInterface to array.
 * Doing so makes the function independent from the config location/source
 */
class AlbumMiddlewareFactory
{
    public function __invoke(ContainerInterface $container)
    {
        $router = $container->get(AlbumRouter::class);
        $app = new AlbumMiddleware($router, $container);

        $albumMiddlewareConfig = $container->get('config')['album_middleware'];

        $this->injectRoutesAndPipeline($app, $albumMiddlewareConfig);
